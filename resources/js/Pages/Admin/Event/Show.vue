...
function uploadPhoto(e){
    ...
    newPhotoForm.post(route('admin.event.addPhoto'), {
        preserveState: true,
        preserveScroll: true,
    });
}
...
function deletePhoto(id){
    router.delete(route('admin.event.deletePhoto', id), {
        preserveState: true,
        preserveScroll: true,
    });
}
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-neutral-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <Link :href="route('admin.establishment.index', estid)" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-neutral-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Назад</Link>
                    <Link :href="route('admin.event.edit.index', {
                        ids: [event.id, estid]
                    })"
